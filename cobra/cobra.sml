...
and parse_stmt tkns = 
  ...
  | "if"     => let val _ = (process "if" tkns)
		    val c = (parse_cond tkns)
		    val _ = (process ":" tkns)
		    val i = (parse_stmt tkns)
		    val _ = (process "else" tkns)
		    val _ = (process ":" tkns)
		    val e = (parse_stmt tkns)
		in 
		    (IF (c, i, e))
		end
...
fun gen_expn x (NUM i)     = [SET (x,i)]
  ...
  | gen_expn x (PLUS (e1,e2))  = 
	let val y = gen_tmp ()
	    val z = gen_tmp ()
	in 
	    (gen_expn y e1) @ (gen_expn z e2) @ [ADD (x, y, z)]
	end
  ...
  | gen_expn x (MINUS (e1,e2)) = 
	let val y = gen_tmp ()
	    val z = gen_tmp ()
	in 
	    (gen_expn y e1) @ (gen_expn z e2) @ [SUB (x, y, z)]
	end
...
fun gen_stmt (ASSIGN (x,e)) = gen_expn x e
  ...
  | gen_stmt (WHILE (LESS  (e1,e2), b)) = 
	let val ch = gen_lbl "on"
	    val bl = gen_lbl "istrue"
	    val dl = gen_lbl "done"
	    val x1 = gen_tmp ()
	    val x2 = gen_tmp ()
	    val x = gen_tmp ()
	    val hdr = [LABEL ch] @ (gen_expn x1 e1) @ (gen_expn x2 e2) @ [SUB(x,x1,x2)]
	    val cjmp = [GOTOIF(x,LT0,bl)]
	    val els = [GOTO dl]
	    val thn = [LABEL bl] @ (gen_stmt b) @ [GOTO ch]
	    val after = [LABEL dl]
	    
	in
  	    hdr @ cjmp @ els @ thn @ after
	end 
